...
namespace {
    ...
    struct JumpscareDownloadDelegate : LevelDownloadDelegate {
        WeakRef<PlayLayer> m_playLayer;
        int m_levelId;
        std::string m_levelName;
        bool m_dontCreateObjects;
        bool m_useReplay;

        JumpscareDownloadDelegate(PlayLayer* pl, int levelId, std::string levelName, bool dontCreateObjects, bool useReplay) : m_playLayer(pl), m_levelId(levelId), m_levelName(std::move(levelName)), m_dontCreateObjects(dontCreateObjects), m_useReplay(useReplay) {}
    ...
    struct JumpscareSearchDelegate : LevelManagerDelegate {
        WeakRef<PlayLayer> m_playLayer;
        int m_levelId;
        std::string m_levelName;
        int m_songId;
        ...
        JumpscareSearchDelegate(PlayLayer* pl, int levelId, std::string levelName, int songId, bool dontCreateObjects, bool useReplay) : m_playLayer(pl), m_levelId(levelId), m_levelName(std::move(levelName)), m_songId(songId), m_dontCreateObjects(dontCreateObjects), m_useReplay(useReplay) {}
        ...
        void loadLevelsFinished(cocos2d::CCArray* levels, char const* key) override {
            ...
            if (auto level = typeinfo_cast<GJGameLevel*>(levels->objectAtIndex(0))) {
                auto delegate = new JumpscareDownloadDelegate(m_playLayer.lock(), m_levelId, m_levelName, m_dontCreateObjects, m_useReplay);
                downloadLevelWithDelegate(level->m_levelID.value(), m_songId, delegate);
            } else {
    ...
    if (auto glm = GameLevelManager::sharedState()) {
        ...
        if (auto storedLevels = glm->getStoredOnlineLevels(search->getKey()); storedLevels && storedLevels->count() > 0) {
            if (auto level = typeinfo_cast<GJGameLevel*>(storedLevels->objectAtIndex(0))) {
                ...
                downloadLevelWithDelegate(level->m_levelID.value(), getJumpscareSongId(levelID), delegate);
                return;
            }
        }
...
void jumpscares::util::download(int levelId, int songId, LevelDownloadDelegate* delegate) {
    if (auto glm = GameLevelManager::sharedState()) {
        if (glm->hasDownloadedLevel(levelId)) {
            ...
            if (auto savedLevel = getSavedDownloadedLevel(levelId)) {
                glm->updateLevel(savedLevel);
            }
        } else {
            downloadLevelWithDelegate(levelId, songId, delegate);
        }
    } else {
